...
from ._exceptions import DecodedVectorizeError, APIResponseValidationError
...
class BaseAPIResponse(Generic[R]):
    ...
    def _parse(self, *, to: type[_T] | None = None) -> R | _T:
        ...
        if (
            ...
            raise TypeError(
                "Pydantic models must subclass our base model type, e.g. `from decoded_vectorize import BaseModel`"
            )
...
class APIResponse(BaseAPIResponse[R]):
    ...
    def parse(self, *, to: type[_T] | None = None) -> R | _T:
        ...
        ```py
        from decoded_vectorize import BaseModel
...
class MissingStreamClassError(TypeError):
    def __init__(self) -> None:
        super().__init__(
            "The `stream` argument was set to `True` but the `stream_cls` argument was not given. See `decoded_vectorize._streaming` for reference",
        )


class StreamAlreadyConsumed(DecodedVectorizeError):
    """
